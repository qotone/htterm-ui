...
static uint32_t keycode_to_ascii_navi_h(uint32_t ir_key);
static uint32_t keycode_to_ascii_navi_v(uint32_t ir_key);
...
bool keypad_read_navi_v(lv_indev_drv_t * indev_drv, lv_indev_data_t * data)
{
    (void) indev_drv;      /*Unused*/
    data->state = state;
    data->key = keycode_to_ascii_navi_v(last_key);

    /*Return `false` because we are not buffering and no more data to read*/
    return false;
}

bool keypad_read_navi_h(lv_indev_drv_t * indev_drv, lv_indev_data_t * data)
{
    (void) indev_drv;      /*Unused*/
    data->state = state;
    data->key = keycode_to_ascii_navi_h(last_key);
...
static uint32_t keycode_to_ascii_navi_v(uint32_t ir_key)
{
    ...
    case REMOTE_KEY_TWO:
        return SDLK_2;
    case REMOTE_KEY_THREE:
        return SDLK_3;
    case REMOTE_KEY_FOUR:
        return SDLK_4;
    case REMOTE_KEY_FIVE:
        return SDLK_5;
    case REMOTE_KEY_SIX:
        return SDLK_6;
    case REMOTE_KEY_SEVEN:
        return SDLK_7;
    case REMOTE_KEY_EIGHT:
        return SDLK_8;
    case REMOTE_KEY_NINE:
        return SDLK_9;
    case REMOTE_KEY_ZERO:
        return SDLK_0;
